...
#define ACCX   //Mouse movement Accelerated; every interrupt processed but accelerated movement, no lag x2-50, 2-50 pulse = 1 move
...
#ifdef ACCX
  //If the encoder has moved 2-50 or more transitions move the mouse in the appropriate direction 
  //and update the position variable to reflect that we moved the mouse. Accelerated move.
  if (rotPositionX >= rotCtrLmt) {
//    rotMultiX = rotPositionX >> 1;
    rotCtr = 0;
...
//    rotPositionX -= (rotMultiX << 1);
  ...
  else if (rotPositionX <= -rotCtrLmt) {
    rotCtr = 0;
    while (rotPositionX <= -rotCtrLmt) {
      rotPositionX += rotCtrLmt;
      rotCtr++; }
...
//      case 4:  //on digital pin 8, PB4 - Arcade Button 5 - L
      case 4:  //on digital pin 8, PB4 - Up Sense Button 
        currentButtonState = (PINB & xPB4) >> 4;
        if (currentButtonState == butPrs)
          adjTime = millis() + delayTime;
        break;
//      case 5:  //on digital pin 9, PB5 - Arcade Button 6 - R
      case 5:  //on digital pin 9, PB5 - Down Sense Button 
...
//If the current state of the pin for each button is different than last time, update the joystick button state
    if(currentButtonState != lastButtonState[button]) { //change in button
      ...
      if (button == upBut || button == dwnBut) {
         proc_SpinSpeed(currentButtonState, button );    
         
         if (adjTime < millis() && currentButtonState != butPrs) {
