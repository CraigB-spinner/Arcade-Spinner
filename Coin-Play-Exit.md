...
Initially both sides of LED will be +5V and Button (SPST switch) supplies Gnd to pull the input low and trigger a button-press and light the LED. The LED +ve will use the red daisy chain wire, LED Gnd will jumper to input pin. Arduino MCUs use common Gnd for inputs which is reverse to Zero Delay USB Encoders. Wiring must adjust to new common. Arduino boards also use Dupont connectors on pin headers.
